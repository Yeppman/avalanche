...
function/* */ersPolyfill(all) {
  // Get/* */ers: implemented according to mozilla's example code: https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/getAllResponseHeaders#Example
...
XHRTransport.prototype.open = function (xhr, onStartCallback, onProgressCallback, onFinishCallback, url, withCredentials,/* */ers) {
  xhr.open('GET', url);
  var offset = 0;
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 2) {
      ...
      var/* */ers = xhr.getAllResponseHeaders();
      onStartCallback(status, statusText, contentType, new/* */ersPolyfill(headers), function () {
        xhr.abort();
      });
  ...
  for (var name in/* */ers) {
    if (Object.prototype.hasOwnProperty.call(headers, name)) {
      xhr.setRequestHeader(name,/* */ers[name]);
    }
  }
...
FetchTransport.prototype.open = function (xhr, onStartCallback, onProgressCallback, onFinishCallback, url, withCredentials,/* */ers) {
  ...
  (0, _unfetch["default"])(url, {
   /* */ers:/* */ers,
    ...
    onStartCallback(response.status, response.statusText, response.headers.get('Content-Type'), new/* */ersWrapper(response.headers), function () {
      controller.abort();
      reader.cancel();
    });
    return new Promise(function (resolve, reject) {
...
function start(es, url, options) {
  ...
  var/* */ers = options != undefined && options.headers != undefined ? JSON.parse(JSON.stringify(options.headers)) : undefined;
  ...
  var onStart = function onStart(status, statusText, contentType,/* */ers, cancel) {
  ...
  var onTimeout = function onTimeout() {
    ...
    // Request/* */er field Last-Event-ID is not allowed by Access-Control-Allow-Headers.
